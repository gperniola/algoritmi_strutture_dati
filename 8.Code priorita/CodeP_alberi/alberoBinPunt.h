...
void AlberoBinPunt<T>::insFiglioSin(nodo n, AlberoBin<T,Cell<T>*>& A){
    if (n->get_figlioSin() == nullptr){
        if (!alberoBinVuoto()){
            ...
            aggiornaLivello(getLivello(n) + 1, A.binRadice());
        } else throw EmptyTree();
...
template <class T>
void AlberoBinPunt<T>::aggiornaLivello(int liv, nodo n){
    n->set_livello(liv);
    if(!sinVuoto(n))
        aggiornaLivello(liv + 1, binFiglioSin(n));
    if(!desVuoto(n))
        aggiornaLivello(liv + 1, binFiglioDes(n));
}
