...
public:
    ...
    virtual void insert_ordered(value_type);
    virtual position linear_search(const value_type &) const;
    virtual position linear_ord_search (value_type &);
...
template <class T, class P>
void Linear_list<T,P>::insert_ordered(value_type val){
...
template <class T, class P>
typename Linear_list<T,P>::position Linear_list<T,P>::linear_ord_search (value_type &val){
    cout << "Inside linear_ord_search():" <<endl;
    cout << val << " --- ";
    Linear_list<T,P>::position pos = begin();
    bool found = false;
    bool not_here = false;
